...
export default function SignUpForm() {
  ...
  const supabase = createClient() // 클라이언트에서 supabase 인스턴스 생성
  ...
  const signUpErrorMessages = {
    ...
    'Server error, please try again later': '서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요.'
  }
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 p-6 shadow-lg rounded-lg">
      <h2 className="text-3xl font-medium text-center mb-6 text-black dark:text-white">회원가입</h2>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormMessage className="text-gray-400 text-xs">{form.formState.errors.email?.message}</FormMessage>
              </FormItem>
            )}
          />
          {/* 비밀번호 입력 */}
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormMessage className="text-gray-400 text-xs">{form.formState.errors.password?.message}</FormMessage>
              </FormItem>
            )}
          />
          {/* 비밀번호 확인 입력 */}
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-lg font-medium">비밀번호 확인</FormLabel>
                <FormControl>
                  <Input
                    type="password"
                    className={`border-b ${
                ...
                <FormMessage className="text-gray-400 text-xs">{form.formState.errors.rePassword?.message}</FormMessage>
              </FormItem>
            )}
          />
          ...
          <Link href="/u/signin" className={cn('w-full', buttonVariants({ variant: 'link' }))}>
            기존 계정으로 로그인
          </Link>
        </form>
      </Form>
